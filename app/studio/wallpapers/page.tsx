...
export default function WallpapersListing() {
  ...
  const fetch = useCallback(async (sort: SortKey) => {
    setLoading(true);
  ...
  useEffect(() => {
    if (!authLoading && !rolesLoading && user && canCreate) fetch(sortBy);
  }, [authLoading, rolesLoading, user, canCreate, fetch, sortBy]);
